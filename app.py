...
# --- بنك الأسئلة التفاعلي (يضاف في النهاية) ---
def show_questions_bank():
    st.markdown("---")
    st.markdown("## 🎯 بنك الأسئلة التفاعلي")
    st.caption("اختر التصنيف المناسب وانسخ السؤال بضغطة زر")

    tab1, tab2, tab3 = st.tabs(["🥊 ركن المناظرة", "💻 مجتمع الـ AI", "🧠 تفكير منطقي"])

    with tab1:
        st.subheader("أسئلة لنسف منطق الجاذبية والانحناء")
        questions_debate = [
            "إذا كان الماء (يبحث دائماً عن مستواه)، فبأي قانون فيزيائي تجريبي يمكن للماء المكشوف أن ينحني أو يتقوس؟",
            "لماذا لا توجد رحلات جوية مباشرة تمر فوق القارة القطبية الجنوبية لربط أستراليا بأمريكا الجنوبية؟",
            "لماذا يرتفع خط الأفق دائماً ليصبح بمستوى عين المشاهد مهما ارتفع في المنطاد أو الطائرة؟",
            "لماذا لا تسحب الجاذبية الدخان أو بخار الماء للأسفل، بينما تسحب مياه المحيطات؟ هل الجاذبية اختيارية؟",
            "على ارتفاع 30 كم، يظهر الأفق مستوياً تماماً في الكاميرات الحقيقية (بدون عدسة واسعة)، أين الانحناء المحسوب؟"
        ]
        for q in questions_debate:
            st.code(q, language=None)
            st.markdown("---")

    with tab2:
        st.subheader("أسئلة تقنية لزيادة التفاعل في حسابك")
        questions_ai = [
            "في Streamlit: هل الأفضل استخدام @st.cache_data للبيانات الحية، أم جلبها Live في كل ثانية؟ ولماذا؟",
            "كيف يمكن محاكاة 'نقطة التلاشي' (Vanishing Point) برمجياً في واجهات المستخدم لتعزيز إحساس العمق؟",
            "هل تتوقع أن يتمكن الـ AI مستقبلاً من كشف صور الـ CGI للأرض بناءً على فيزياء الضوء والظلال؟",
            "ماهو أفضل Framework لبناء Dashboards سريعة لمشاريع الـ AI في 2026؟"
        ]
        for q in questions_ai:
            st.info(q)
            st.button("📋 نسخ السؤال", key=f"btn_{q[:20]}", help="اضغط لنسخ السؤال")

    with tab3:
        st.subheader("أسئلة تفتح باب التفكير الحر")
        questions_logic = [
            "أيهما أولى بالتصديق: الحواس الخمس التي تؤكد ثبات الأرض، أم النظريات التي تفترض حركتها؟",
            "لماذا لا نشعر بدوران الأرض بسرعة 1600 كم/س في استقرار مياه الأكواب أو حركة الطائرات؟",
            "لماذا تظل البروج (الجوزاء والحمل) في مساراتها الثابتة منذ آلاف السنين رغم حركة الكون المزعومة؟"
        ]
        for q in questions_logic:
            st.warning(q)
